...
def get_quality_score(features, model):
	...
	score = pca.transform(features_scaled)[0, 0] * pca.explained_variance_ratio_[0] + pca.transform(features_scaled)[0, 1] * pca.explained_variance_ratio_[1]
	return score
...
def single_method_eval(img, mask, output_dir: Path) -> Tuple[Dict[str, Any], float, float]:
	...
	for channel in range(metric_mask.shape[0]):
		...
		if channel_names[channel] == "Matched Cell":
			...
			metrics[channel_names[channel]]["1/(ln(StandardDeviationOfCellSize)+1)"] = 1 / (np.log(cell_size_std) + 1)
			metrics[channel_names[channel]]["FractionOfMatchedCellsAndNuclei"] = matched_fraction
			metrics[channel_names[channel]]["1/(AvgCVForegroundOutsideCells+1)"] = 1 / (foreground_CV + 1)
			metrics[channel_names[channel]][
